...
export function writePostcard(sender, receiver, message) {
  ...
  else if(sender.trim() === "" || receiver.trim() === "" || message.trim() === "") return ""
...
export function isValidPincode(code) {
  ...
  else if(code.startsWith("0") || code.length !== 6) return false
  else if(!/^\d+$/.test(code)) return false
  else return true
}
...
export function formatPostcardField(label, value, width) {
  ...
  const paddedLabel = label.padEnd(width || 12)
