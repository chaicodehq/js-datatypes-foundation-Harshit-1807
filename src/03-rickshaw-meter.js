...
export function roundFare(amount, decimalPlaces) {
  ...
  else if(decimalPlaces < 0 || !Number.isInteger(decimalPlaces)) return ""
  else {
...
export function calculateSurge(baseFare, surgeMultiplier) {
  ...
  else return Math.ceil(baseFare * surgeMultiplier)
}
...
export function findCheapestAndCostliest(...fares) {
  ...
  else return { cheapest: Math.min(...validFares), costliest: Math.max(...validFares) }
}

export function getDistanceDifference(from, to) {
  // Your code here
  let fromKm = parseInt(from);
  let toKm = parseInt(to);
  if(isNaN(fromKm) || isNaN(toKm)) return -1;
  return Math.abs(toKm - fromKm);
}
