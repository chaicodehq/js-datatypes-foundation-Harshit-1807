...
export function repeatPattern(pattern, times) {
  // Your code here
  if(typeof(pattern) !== 'string' || typeof(times) !== 'number' || !Number.isInteger(times)) return ""
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  ...
  else return colorString.split(oldSep).join(newSep);
}
...
export function makeRangoliBorder(char, length) {
  ...
  else return char.repeat(length).slice(0, length);
}
